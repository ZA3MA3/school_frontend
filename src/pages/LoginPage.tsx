...
export default function LoginPage() {
  ...
  if (needsSubscription) {
    ...
    if (hasTeacher && hasParent) {
      renderContent = 'TEACHER_PARENT';
    } else if (hasTeacher) {
      renderContent = 'TEACHER_ONLY';
    } else if (hasParent) {
      renderContent = 'PARENT_ONLY';
    }
  }

  // Render subscription payment if needed
  if (needsSubscription) {
    let planName = '';
    let planPrice = '';
    let planType = '';
    
    if (renderContent === 'TEACHER_PARENT') {
      planType = 'TEACHER_PARENT';
      planName = 'Teacher + Parent Plan';
      planPrice = '5,000 DZD';
    } else if (renderContent === 'TEACHER_ONLY') {
      planType = 'TEACHER_ONLY';
      planName = 'Teacher Plan';
      planPrice = '3,000 DZD';
    } else if (renderContent === 'PARENT_ONLY') {
...
<Card className="w-full max-w-md dark:bg-zinc-800 relative z-10">
  ...
  // Check subscription - show payment page if no active subscription
  if (needsSubscription) {
    const hasTeacher = userRoles.includes('TEACHER');
    const hasParent = userRoles.includes('PARENT');
...
export default function LoginPage() {
  ...
  if (needsSubscription) {
    const hasTeacher = userRoles.includes('TEACHER');
    const hasParent = userRoles.includes('PARENT');
    console.log("haha",hasTeacher,hasParent)
    let planType = '';
    let planName = '';
    let planPrice = '';
